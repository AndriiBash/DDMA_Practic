...
void MainWindow::on_calculate_clicked()
{
    ...
    QAxObject *charts = sheet->querySubObject("ChartObjects()");
    QAxObject *chart = charts->querySubObject("Add(double,double,double,double)", 100, 50, 400, 300);
    QAxObject *chartInstance = chart->querySubObject("Chart");
    chartInstance->setProperty("ChartType", 73); // 73 - Column Clustered

    QAxObject *xvalues = sheet->querySubObject("Range(const QString&)", "A2:A9");
    QAxObject *yvalues = sheet->querySubObject("Range(const QString&)", "B2:B9");

    // Додаємо серії
    QAxObject *series = chartInstance->querySubObject("SeriesCollection");
    QAxObject *newSeries = series->querySubObject("NewSeries()");
    newSeries->setProperty("XValues", xvalues->asVariant());
    newSeries->setProperty("Values", yvalues->asVariant());

    // Перевіряємо кількість серій
    int count = series->dynamicCall("Count()").toInt();
    if (count < 1) {
        qDebug() << "SeriesCollection is empty.";
        return;
    }

    // Збереження та закриття
    workbook->dynamicCall("SaveAs(const QString&)", "D:\\test\\test.xlsx");
